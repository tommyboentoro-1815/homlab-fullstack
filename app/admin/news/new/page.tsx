...
export default function NewArticle() {
  ...
  const [form, setForm] = useState({
    title: '', date: '', excerpt: '', subheading: '', author: '', tags: ''
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (paragraphs.every(p => !p.trim())) { setError('Please add at least one paragraph'); return }
    setLoading(true)
    setError('')
    ...
    else { setError('Failed to save article'); setLoading(false) }
  }
  ...
  return (
    <div style={{ maxWidth: '720px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px' }}>
        ...
        <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#1a1a1a', margin: 0 }}>Add Article</h1>
      </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
            {images.map((img, i) => (
              <div key={i} style={{ position: 'relative' }}>
                <img src={img.preview} alt="" style={thumbStyle} />
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              ...
              <input required style={inputStyle} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Article title" />
            </div>
            <div>
